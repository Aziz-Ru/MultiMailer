...
import getSingleEmail, { sendEmail, updateEmail } from "./api";

const SingleEmail = ({ params }) => {
  const { id } = params;
  ...
  useEffect(() => {
    ...
    const getEmail = async () => {
      setLoading(true)
      const response = await getSingleEmail(id);
      if (response.id) {
        setSavedata({
          ...Savedata,
          subject: response.subject,
          email_text: response.email,
        });
        setFlags({
          ...Flags,
  ...
  const updateData = async () => {
    ...
    const payload = {
      email: Savedata.email_text,
      subject: Savedata.subject,
    };
    setLoading(true)
    const response = await updateEmail(id, payload);
  ...
  const onChangeEmails = (e) => {
    ...
    for (const email of emails) {
      ...
      if (!allowedDomains.some((domain) => trimmedEmail.includes(domain))) {
        setFlags({
          ...Flags,
          message: `${trimmedEmail} is not a valid email`,
        });
        return;
      }
  ...
  return (
    <>
      ...
      <div className="md:ml-52 mt-20 ">
        ...
        <div>
        <form action="" className={`${Loading ? "opacity-20" : ""}`}>
            ...
            <div className="mb-2 mx-10">
              <textarea
                ...
                } bg-gray-50 border-gray-300 text-gray-900 text-lg  rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full h-[60vh] py-2 px-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
              ></textarea>
            </div>
            ...
            <div className="flex mx-10 gap-4 justify-center mb-2">
              <button
                ...
                className={`${
                  Loading ? "cursor-not-allowed" : ""
                } text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xl w-96 px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800`}
              >
                Update
              </button>
              <button
                ...
                } text-white w-96 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xl px-4 py-2.5  text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800`}
              >
                Send
              </button>
            </div>
          ...
          {Loading && (
            <div
              role="status"
              className=" bg-black absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
            >
              <svg
                aria-hidden="true"
